...
export default {
  ...
  mounted() {
    var _this = this;
    _this.$nextTick(() => {
      this.$refs.contain.addEventListener("scroll", function() {
        let viewH = this.clientHeight; //可见高度
        let contentH = this.scrollHeight; //内容高度
        let scrollTop = this.scrollTop; //滚动高度
        if (scrollTop > viewH + 100) {
          _this.showTop = true;
        } else {
          _this.showTop = false;
        }
